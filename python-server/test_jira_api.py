...
def test_connectivity():
    ...
    try:
        ...
        if is_connected:
            ...
            print(f"{Colors.GREEN}✓ Logged in as: {user.get('displayName')} ({user.get('emailAddress')}){Colors.END}")
            return True
        else:
...
def test_issues(project_key):
    ...
    try:
        ...
        if issues and "issues" in issues and issues["issues"]:
            ...
            print(f"{Colors.GREEN}✓ Successfully retrieved {len(issue_list)} issues{Colors.END}")
            ...
            for i, issue in enumerate(issue_list):
                ...
                status_color = Colors.GREEN if status.lower() == "done" else Colors.YELLOW if status.lower() == "in progress" else Colors.BLUE
                print(f"  {i+1}. {issue['key']} - {fields.get('summary', 'No summary')} | {status_color}{status}{Colors.END} | Assignee: {assignee}")
            
            return issues
        else:
            print(f"{Colors.YELLOW}! No issues found in project {project_key}{Colors.END}")
            return {}
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if not token:
        print(f"{Colors.RED}No OAuth token available. Please authenticate first.{Colors.END}")
        sys.exit(1)
