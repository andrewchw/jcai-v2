...
def test_event_handler(event: TokenRefreshEvent):
    ...
    print(
        f"{color}{event.timestamp}: [{event.event_type.upper()}] {event.message}{Colors.END}"
    )
...
def display_token_info(token):
    ...
    if "access_token" in masked_token:
        ...
        masked_token["access_token"] = (
            token_val[:5] + "..." + token_val[-5:] if len(token_val) > 10 else "***"
        )
    if "refresh_token" in masked_token:
        token_val = masked_token["refresh_token"]
        masked_token["refresh_token"] = (
            token_val[:5] + "..." + token_val[-5:] if len(token_val) > 10 else "***"
        )
    ...
    if "expires_at" in token:
        ...
        print(
            f"{Colors.BOLD}Expiration:{Colors.END} {expires_text} ({expiry_time.strftime('%Y-%m-%d %H:%M:%S')})"
        )
...
def display_service_stats():
    ...
    if stats["next_scheduled_check"]:
        ...
        if next_check > now:
            ...
            print(
                f"  Next Check: {int(wait_time)} seconds from now ({next_check.strftime('%Y-%m-%d %H:%M:%S')})"
            )
        else:
        ...
        try:
            while True:
                ...
                try:
                    ...
                    print(
                        f"{Colors.RED}× Error checking Jira connection: {str(e)}{Colors.END}"
                    )
                ...
                if os.name == "nt":  # Windows
                    ...
                    if msvcrt.kbhit():
                        ...
                        if key == "q":
                            ...
                            print(
                                f"{Colors.YELLOW}Forcing token refresh...{Colors.END}"
                            )
                            new_token = jira_service.refresh_oauth2_token(force=True)
                            if new_token:
                                print(
                                    f"{Colors.GREEN}Token refreshed successfully!{Colors.END}"
                                )
                            else:
                            ...
                            print(
                                f"{Colors.YELLOW}Testing Jira API by retrieving projects...{Colors.END}"
                            )
                            try:
                                ...
                                for i, project in enumerate(
                                    ...
                                    print(
                                        f"  {i+1}. {project['name']} ({project['key']})"
                                    )
                                if len(projects) > 5:
                                    print(f"  ... and {len(projects) - 5} more")
                            except Exception as e:
                                print(
                                    f"{Colors.RED}Error retrieving Jira projects: {str(e)}{Colors.END}"
                                )
