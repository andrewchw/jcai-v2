        ...
        if 'expires_at' in token:
            ...
            if time_remaining <= 60:  # Within 60 seconds of expiration or already expired
                ...
                if 'expires_at' in new_token:
                    ...
                    print(f"✅ Token refreshed successfully! New expiration: {new_expiry.strftime('%Y-%m-%d %H:%M:%S')}")
...
def get_accessible_resources(token):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(f"Error accessing resources: {response.status_code} - {response.text}")
            return None
