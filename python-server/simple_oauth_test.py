        ...
        if is_authenticated and token_present:
            ...
            try:
                ...
                if debug_response.status_code != 200:
                    print(
                        f"ERROR: Could not access debug API. Status code: {debug_response.status_code}"
                    )
                    ...
                    if config_response.status_code == 200:
                        ...
                        print(
                            f"Callback URL: {config.get('callback_url', 'Not available')}"
                        )
                        print(
                            f"Cloud ID from env: {config.get('cloud_id', 'Not available')}"
                        )
                    else:
                        print(
            ...
            try:
                user_url = (
                    f"http://localhost:8000/api/jira/v2/user?user_id={quote(user_id)}"
                )
                user_response = requests.get(user_url)

                if user_response.status_code != 200:
                    print(
                        f"ERROR: Could not retrieve user information. Status code: {user_response.status_code}"
                    )
            ...
            try:
                ...
                if response.status_code != 200:
                    print(
                        f"ERROR: Could not access Jira projects API. Status code: {response.status_code}"
                    )
                    ...
                    if len(projects) > 0:
                        ...
                        for idx, project in enumerate(
                            ...
                            print(
                                f"{idx+1}. {project.get('key')}: {project.get('name')}"
                            )
            except Exception as e:
