...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('oauth_token_service.log')
    ]
)
...
class TokenRefreshEvent:
    ...
    def __init__(self, event_type: str, message: str, token_info: Dict[str, Any] = None):
...
class OAuthTokenService:
    ...
    def _notify_event(self, event_type: str, message: str, token_info: Dict[str, Any] = None):
    ...
    def refresh_token(self, token: Dict[str, Any], force: bool = False) -> Optional[Dict[str, Any]]:
        ...
        if not token or 'refresh_token' not in token:
            ...
            self._notify_event("error", "Cannot refresh token: No refresh token available")
            return None
        ...
        if 'expires_at' in token and not force:
            ...
            if time_remaining <= self.refresh_threshold:
                needs_refresh = True
                if time_remaining <= 0:
                    logger.info(f"Token expired {timedelta(seconds=abs(time_remaining))} ago")
                else:
                    logger.info(f"Token expires in {timedelta(seconds=time_remaining)}, within threshold")
        ...
        while retry_count <= self.max_retries:
            try:
                ...
                if retry_count == 0:
                    if force:
                        ...
                        if time_remaining and time_remaining <= 0:
                            ...
                            self._notify_event("info", f"Token expired, refreshing")
                        else:
                            logger.info(f"Token expires soon, refreshing")
                            self._notify_event("info", f"Token expires soon, refreshing")
                else:
                    logger.info(f"Retrying token refresh (attempt {retry_count}/{self.max_retries})")
                ...
                retry_count += 1
                logger.error(f"Error refreshing token (attempt {retry_count}/{self.max_retries}): {str(e)}")
                ...
                if retry_count <= self.max_retries:
                    ...
                    self._notify_event("error", f"Token refresh failed after {retry_count} attempts: {str(e)}")
            ...
            while not self._stop_event.is_set():
                try:
                    ...
                    if not token:
                        ...
                        self._notify_event("warning", "No token available for background refresh")
                    else:
                        ...
                        if 'expires_at' in token:
                            ...
                            if time_remaining <= self.refresh_threshold:
                                ...
                                time_to_refresh = time_remaining - self.refresh_threshold
                                
                                # If next check would miss the refresh window, adjust it
                                if time_to_refresh < self.check_interval:
                                    next_check = max(60, int(time_to_refresh))  # Min 1 minute
                                    logger.info(f"Adjusting next check to {next_check} seconds to catch refresh window")
                                
                                logger.info(f"Token valid for {timedelta(seconds=time_remaining)}, next check in {next_check} seconds")
                                
                                # Calculate and store next check time
                                self.stats["next_scheduled_check"] = datetime.now() + timedelta(seconds=next_check)
                            ...
                            self._notify_event("warning", "Token has no expiration information")
                    
                    # Standard check interval
                    self.stats["next_scheduled_check"] = datetime.now() + timedelta(seconds=self.check_interval)
...
if __name__ == "__main__":
    ...
    try:
        ...
        while True:
            print("\nService status:")
            print(f"- Refreshes attempted: {token_service.stats['refreshes_attempted']}")
            print(f"- Refreshes succeeded: {token_service.stats['refreshes_succeeded']}")
            print(f"- Refreshes failed: {token_service.stats['refreshes_failed']}")
            
            if token_service.stats["last_refresh"]:
                print(f"- Last refresh: {token_service.stats['last_refresh'].strftime('%Y-%m-%d %H:%M:%S')}")
            ...
            if token_service.stats["next_scheduled_check"]:
                ...
                if next_check > now:
                    ...
                    print(f"- Next check: {time_to_check:.0f} seconds from now ({next_check.strftime('%Y-%m-%d %H:%M:%S')})")
                else:
            ...
            if token and 'expires_at' in token:
                ...
                if time_remaining <= 0:
                    print(f"- Token status: EXPIRED ({abs(time_remaining):.0f} seconds ago)")
                else:
                    print(f"- Token status: valid for {time_remaining:.0f} seconds more")
                    
                    # Show if within refresh threshold
                    if time_remaining <= token_service.refresh_threshold:
                        print(f"  (Within refresh threshold of {token_service.refresh_threshold} seconds)")
