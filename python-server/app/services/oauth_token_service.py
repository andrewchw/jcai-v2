...
class OAuthTokenService:
    ...
    def __init__(
        ...
        self._event_history: List[TokenRefreshEvent] = []  # Statistics
    ...
    def start(self):
        ...
        if OAuthTokenService._refresh_thread_running:
            logger.debug(
                "OAuth token refresh service already running - using existing instance"
            )
            return
    ...
    def refresh_token(
        ...
        while retry_count <= self.max_retries:
            try:
                self.stats["refreshes_attempted"] = (
                    int(self.stats["refreshes_attempted"]) + 1
                )  # Log refresh attempt
                if retry_count == 0:
                    if force:
                ...
                # Update stats
                self.stats["refreshes_succeeded"] = (
                    int(self.stats["refreshes_succeeded"]) + 1
                )
                ...
                if retry_count <= self.max_retries:
                    ...
                    self.stats["refreshes_failed"] = (
                        int(self.stats["refreshes_failed"]) + 1
                    )
                    self._notify_event(
                        "error",
...
if (
    ...
    if not CLIENT_ID or not CLIENT_SECRET:
        print(
            "Error: JIRA_OAUTH_CLIENT_ID and JIRA_OAUTH_CLIENT_SECRET environment variables required"
        )
        exit(1)
    ...
    try:
        # Keep main thread alive for demonstration        while True:
        print("\nService status:")
