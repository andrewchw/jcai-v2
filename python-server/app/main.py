...
async def root_oauth_callback(
    ...
    print(f"OAuth callback received at root level: state={state}, code={'present' if code else 'missing'}, user_id={user_id}")
    
    # Forward to the actual callback handler in oauth_multi with the extracted parameters and db session
    return await oauth_multi_callback(request=request, user_id=user_id, code=code, state=state, setup_example=setup_example, success=success, db=db)
...
async def startup_event():
    ...
    if not encryption_key:
        logger.warning("No encryption key found in environment. A new key will be generated.")
    else:
        logger.info("Encryption key found in environment")
    
    # Check multi-user flag
    multi_user_enabled = os.environ.get("JIRA_ENABLE_MULTI_USER", "true").lower() == "true"
...
@app.get("/api/health")
async def health_check(user_id: str = None, db: Session = Depends(get_db)):
    ...
    database_info = {
        ...
        "type": "sqlite" if os.environ.get("DATABASE_URL", "").startswith("sqlite") else "custom"
    }
        ...
        if user_id:
            ...
            if jira_service:
                ...
                token_info = {
                    ...
                    "multi_user": True,
                    "expires_in": int(token.get("expires_at", 0) - datetime.now().timestamp()) if token else None
                }
                debug_info["user_id"] = user_id
                debug_info["user_type"] = "authenticated" if is_authenticated else "unauthenticated"
...
async def root():
    return {
        ...
        "version": "0.2.0", 
        "features": ["Multi-user authentication", "Token encryption", "Database storage"],
        "docs_url": "/docs",
