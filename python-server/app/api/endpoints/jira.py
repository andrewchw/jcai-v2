...
async def health_check():
    ...
    return {
        ...
        "message": "Jira API is connected" if is_connected else "Jira API is not connected"
    }
...
async def get_issue(issue_key: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND if "Issue does not exist" in str(e) else status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def add_comment(issue_key: str, comment: JiraComment):
    ...
    try:
        result = jira_service.add_comment(
            issue_key=issue_key,
