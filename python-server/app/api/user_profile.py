...
user_profile_router = APIRouter(prefix="/user", tags=["user"])
    ...
    try:
        ...
        if user_token_entry:
            ...
            if user_token_entry.access_token:
                logger.info(
                    f"Token successfully retrieved and decrypted for user_id {user_id} via direct model query."
                )
            else:
                logger.warning(
                    f"Token entry found for user_id {user_id}, but decrypted access_token is None."
                )
                token_data = None
        else:
            logger.warning(
                f"No OAuthToken entry found for user_id {user_id} in the database."
            )
    except Exception as e:
        logger.exception(
            f"Error querying/processing OAuthToken for user_id {user_id}: {e}"
        )
        return None
...
async def get_user_profile(user_id: str = Query(...), db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not token_data or not token_data.get("access_token"):
            logger.warning(
                f"No valid access token could be retrieved for user_id: {user_id}. Token data: {token_data}"
            )
            raise HTTPException(
                status_code=401,
                detail="User not authenticated or access token not found",
            )
        ...
        jira_client = JiraService(
            access_token=access_token, user_id=user_id, db_session=db
        )
        ...
        if user_details and user_details.get("displayName"):
            ...
            logger.info(
                f"Successfully fetched profile for user_id {user_id}: {response_data.get('displayName')}"
            )
            return response_data
        else:
            error_message = (
                "Failed to retrieve user details from Jira or displayName missing."
            )
            logger.warning(
                f"{error_message} Jira response: {str(user_details)[:200]} for user_id: {user_id}"
            )
        ...
        logger.exception(
            f"Exception in get_user_profile for user_id {user_id}: {str(e)}"
        )
        raise HTTPException(
