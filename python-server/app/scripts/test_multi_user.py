...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('multi_user_test.log')
    ]
)
...
def simulate_oauth_flow(user_id):
    ...
    try:
        ...
        if retrieved_token:
            ...
            logger.info(f"Token expires at: {datetime.fromtimestamp(retrieved_token.expires_at)}")
            return True
        else:
...
def run_tests():
    ...
    logger.info(f"3. Check token status: GET /api/auth/oauth/v2/status?user_id={user_id}")
    logger.info(f"4. Try listing Jira projects: GET /api/jira/v2/projects?user_id={user_id}")
