...
def get_test_users_info():
    ...
    test_users = []
    user_count = int(
        input("How many Jira test users do you want to test with? (1-3): ")
    )
    ...
    for i in range(1, user_count + 1):
        print(f"\nTest User {i}:")
        email = input("  Email address for User {}: ".format(i))
        username = input("  Username (if different from email): ")
        description = input(
            f"  Brief description (e.g., 'Admin User', 'Regular User'): "
        )
...
def check_token_status(user):
    ...
    try:
        ...
        if is_authenticated and token_present:
            ...
            logger.warning(
                f"User {user_id} ({user['email']}) authentication incomplete. Token present: {token_present}"
            )
            return False
...
def verify_jira_access(user):
    ...
    try:
        ...
        logger.info(
            f"Successfully retrieved {project_count} projects for user {user_id} ({user['email']})"
        )
        print(
