...
def check_token_status(user_id):
    ...
    try:
        ...
        if is_authenticated and token_present:
            ...
            logger.warning(f"User {user_id} authentication incomplete. Token present: {token_present}")
            return False
...
def verify_jira_access(user_id):
    ...
    try:
        ...
        logger.info(f"Successfully retrieved {project_count} projects for user {user_id}")
