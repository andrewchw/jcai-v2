...
class DiagnosticTool:
    def __init__(self, fix_issues=False, verbose=False):
        ...
        self.extension_dir = os.path.join(os.path.dirname(self.server_dir), 'edge-extension')
        
        # Files to check
        self.oauth_service_path = os.path.join(self.server_dir, 'app', 'services', 'oauth_token_service.py')
        self.background_js_path = os.path.join(self.extension_dir, 'src', 'js', 'background.js')
        self.sidebar_js_path = os.path.join(self.extension_dir, 'src', 'js', 'sidebar.js')
        self.jira_multi_path = os.path.join(self.server_dir, 'app', 'api', 'endpoints', 'jira_multi.py')
    ...
    def run_diagnostics(self):
        ...
        if self.issues_found == 0:
            ...
            if self.issues_found > self.issues_fixed:
                logger.warning("WARNING - " + str(self.issues_found - self.issues_fixed) + " issue(s) require manual attention.")
            else:
    ...
    def check_oauth_service(self):
        ...
        if "_refresh_thread_running" not in content:
            ...
            if self.fix_issues:
                logger.info("Attempting to fix multiple token refresh services issue...")
                try:
                    subprocess.run(['python', os.path.join(self.server_dir, 'fix_token_service_direct.py')], 
                                    check=True, cwd=self.server_dir)
    ...
    def check_background_js(self):
        ...
        if len(start_token_calls) > 6:  # More lenient threshold since calls from different contexts are ok
            logger.warning("WARNING - Excessive calls to startTokenChecking found: " + str(len(start_token_calls)))
            self.issues_found += 1
        else:
            if self.verbose:
                logger.info("OK - Appropriate number of startTokenChecking calls found.")
    ...
    def check_sidebar_js(self):
        ...
        with open(self.sidebar_js_path, 'r', encoding='utf-8') as file:
            content = file.read()        # Check for JQL handling in loadTasks - use a different approach to find the function
        ...
        if "function loadTasks()" in content:
            ...
            if load_tasks_start != -1:
                ...
                if "filters.jql" not in after_func and "jqlBase" not in after_func:
                    ...
                    if self.verbose:
                        logger.info("OK - JQL query construction looks good in loadTasks.")
            else:
        ...
        has_debounce = (
            ...
            "lastAuthTime" in content or
            "debounceTime" in content or
            "projectsDebounceTime" in content
        )
        ...
        if "timeSinceAuth < 30000" not in content:
            logger.warning("WARNING - Smart authentication responsiveness may be missing.")
            self.issues_found += 1
        else:
            if self.verbose:
        ...
        if os.path.exists(token_file):
            try:
                ...
                if 'expires_at' in token_data:
                    ...
                    expires_at = datetime.now().timestamp() + token_data['expires_in_seconds']
                    expires_at = datetime.fromtimestamp(expires_at)
                
                if expires_at:
                    now = datetime.now()
                    if expires_at < now:
                        logger.warning(f"⚠️ OAuth token is expired! Expired at {expires_at}")
                        self.issues_found += 1
                    else:
                        time_left = expires_at - now
                        if self.verbose:
                            logger.info(f"✅ OAuth token is valid. Expires in {time_left.total_seconds()/3600:.1f} hours.")
                else:
            ...
            logger.warning("⚠️ No OAuth token file found. The extension may need authentication.")
            self.issues_found += 1

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Jira Chatbot Extension Diagnostic Tool')
