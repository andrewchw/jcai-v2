...
def refresh_token(token):
    ...
    try:
        ...
        if "expires_at" in new_token:
            ...
            refresh_status[
                "message"
            ] = f"✅ Token refreshed! New expiration: {new_expiry.strftime('%Y-%m-%d %H:%M:%S')}"
    ...
    try:
        while True:
            ...
            if "expires_at" in token:
                ...
                expiry_progress = min(
                    1.0, max(0, 1 - time_to_expiry / 3600)
                )  # Assuming 1hr token life
                refresh_progress = min(
                    1.0, max(0, 1 - time_to_refresh / 3600)
                )  # Assuming 1hr token life

                expiry_bar = "█" * int(expiry_progress * 20) + "░" * (
                    20 - int(expiry_progress * 20)
                )
                refresh_bar = "█" * int(refresh_progress * 20) + "░" * (
                    20 - int(refresh_progress * 20)
                )
                ...
                if "refresh_token" in token:
                    print(
                        f"Refresh Token: {token.get('refresh_token', 'Unknown')[:5]}..."
                    )
