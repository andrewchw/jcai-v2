...
def login():
    ...
    authorization_url, state = oauth.authorization_url(
        AUTH_BASE_URL, audience="api.atlassian.com"
    )
...
async def callback(request: Request):
    ...
    if not state or state != session_store.get("oauth_state"):
        logger.error(
            f"State mismatch! Expected: {session_store.get('oauth_state')}, Got: {state}"
        )
        return JSONResponse(
            {
                "status": "error",
    ...
    try:
        ...
        jira_oauth = OAuth2Session(
            CLIENT_ID, redirect_uri=REDIRECT_URI, state=session_store.get("oauth_state")
        )

        # Log the data we're sending for the token request
        logger.info("Exchanging code for token...")
        logger.info(
            f"Code: {code[:10]}...{code[-10:]} (truncated)"
        )  # Partial code for security
        ...
        expiry_time = "unknown"
        if "expires_at" in token:
            expiry_time = datetime.fromtimestamp(token["expires_at"]).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
        ...
        try:
            ...
            if resources:
                ...
                logger.info(
                    f"Updated .env file with ATLASSIAN_OAUTH_CLOUD_ID={cloud_id}"
                )
            else:
...
if __name__ == "__main__":
    ...
    logger.info("=" * 60)
    logger.info(
        f"Client ID: {CLIENT_ID[:5] + '...' + CLIENT_ID[-5:] if CLIENT_ID and len(CLIENT_ID) >= 10 else CLIENT_ID or 'Not set'}"
    )
